...
from gidi_env.gidi_sim.env import env as envs
import torch
import numpy as np
import pandas as pd
...
class pfrnn(nn.Module):
    def __init__(self):
        ...
        self.hidden2label = nn.Linear(self.hidden_dim, 12)
    ...
    def step(self,x,y, pred=None, particle_pred=None):
        ...
        l2_a_loss = torch.sum(l2_pred_loss[:, :, :])
        l2_b_loss = 0

        l2_loss =  h_weight * l2_a_loss + b_weight * l2_b_loss

        l1_a_loss = torch.mean(l1_pred_loss[:, :, :])
        l1_b_loss =0
        ...
        z = torch.from_numpy(np.array(y)).float().view([batch_size,1,12])

        #for i in z[:,0,12:]:
         #   print(i)#print(z[1,0,12:])# z[150,1,12:])
        loss_last = torch.nn.functional.mse_loss(pred[:,:,:], z[:,:,:]) * h_weight
        loss_last += 0
        particle_pred = particle_pred.view(self.num_particles, 12, batch_size)
...
def train_pfrnn(model,STEP_SIZE, EPOCHS):
    ...
    while env.epoch < EPOCHS:
        ...
        pred, pred_pf = torch.empty((1,1,12)), torch.empty((1,384,12))
        ...
        if env.epoch % 5000 == 0:
            ...
            pd.DataFrame(prog).to_csv("output/"+str(env.epoch)+".csv",header=False,index=False)
        
    pd.DataFrame(rwd).to_csv('output/rewards_no_act_12.csv',header=False,index=False)
    ...
    with torch.no_grad():
        ...
        while not done:
            ...
            act = np.ones(12) * .1#np.array([c.detach().numpy()[0][0][12:]])[0]
