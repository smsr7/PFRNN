...
class env():
    ...
    def reset(self, init_time = 0, env_step = None, testing = False, max_tick =5000):
        ...
        self.DATASET  = pd.read_csv("gidi_env/gidi_sim/out0/" + str(env_step) + ".csv", index_col=None, header=None).iloc[1:]
        #print(self.DATASET)
        #try:
...
if __name__ == "__main__":
    ...
    while not done:
        ...
        s,done = env.step(np.ones(12)*.05)
    '''for ii in range(3,10):
        for i in range(1,200):
            print(ii,i)
            env.TESTS = i*1
            env.output = str(ii) + "_" + str(env.TESTS)
            env.reset(testing=True)
