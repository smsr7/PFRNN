...
class env():
    def __init__(self, profile = False):
        ...
        self.epoch = 0
        #hyper params
        self.FIDS       = np.sort(np.array([87])) #87,41,127,135,760,145,7,53,149,183,36,85
    ...
    def reset(self, init_time = 0, env_step = None, testing = False):
        '''
        ...
        if(testing):
            env_step = 29
            self.test = True
            pd.DataFrame([]).to_csv('output/progress_single.csv',header=False,index=False)
         
        else:
            self.test = False

            
        self.DATASET  = pd.read_csv("gidi_env/gidi_sim/out0/" + str(env_step) + ".csv", index_col=None, header=None).iloc[1:][self.iteration]
    ...
    def step(self, action):
        ...
        if action < .1:
            action = .1
        if self.cur_step == len(self.DATASET) - 1:
            self.done = True
        ...
        if self.test:
            ...
            pd.DataFrame([np.asarray(s_new/tests)]).to_csv('output/progress_single'+str(self.iteration)+'.csv', mode='a',header=False,index=False)
