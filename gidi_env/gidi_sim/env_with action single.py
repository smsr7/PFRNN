...
class env():
    ...
    def reset(self, init_time = 0, env_step = None, testing = False):
        ...
        if(env_step == None):
            env_step = random.randint(0,47)

        if(testing):
            env_step = 48
            self.test = True
            pd.DataFrame([]).to_csv('output/progress.csv',header=False,index=False)
    ...
    def step(self, action):
        ...
        if self.test:
            ...
            pd.DataFrame([np.asarray(s_new/tests)]).to_csv('output/progress.csv', mode='a',header=False,index=False)
