...
class env():
    ...
    def reset(self, init_time = 0, env_step = None, testing = False, max_tick =5000):
        ...
        self.DATASET  = pd.read_csv("gidi_env/gidi_sim/out/" + str(env_step) + ".csv", index_col=None, header=None).iloc[1:] + .001
        #print(self.DATASET)
        #try:
        ...
        pd.DataFrame([]).to_csv('output/baseline/'+str(self.output)+'.csv',header=False,index=False)
    ...
    def step(self, action):
        ...
        if self.test:
            ...
            pd.DataFrame([np.asarray(s_new/tests)]).to_csv('output/'+str(self.output)+'.csv', mode='a',header=False,index=False)
